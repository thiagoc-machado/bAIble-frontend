                ...
                <v-avatar class="mx-4" size="40">
                    <v-img
                        ...
                        @error="
                            (e) =>
                                (e.target.src =
                                    '/images/characters/avatar-neutro.png')
                        "
                    >
...
const toggleListening = () => {
  ...
  if (isListening.value) {
    ...
    newMessage.valueBase = newMessage.value // Salva o que já estava escrito
    recognition.lang = getLangFromLocale(locale.value)
    recognition.start()
  }
};
...
const initRecognition = () => {
    ...
    recognition.onresult = (event) => {
        ...
        newMessage.value =
            newMessage.valueBase + finalTranscript + interimTranscript;
    };
...
const getLangFromLocale = (loc) => {
    if (loc.startsWith("pt")) return "pt-PT";
    if (loc.startsWith("es")) return "es-ES";
    return "en-US";
}; // Detecta o idioma da interface e ajusta o reconhecimento de voz
const handleSend = () => {
    sendMessage();
    // Esconde o teclado em dispositivos móveis
    document.activeElement.blur();
...
onMounted(() => {
    ...
    if ("visualViewport" in window) {
        const updateViewport = () => {
            ...
            if (messagesContainer.value) {
                const headerHeight =
                    document.querySelector(".chat-header")?.offsetHeight || 0;
                const footerHeight =
                    document.querySelector(".chat-footer")?.offsetHeight || 0;
